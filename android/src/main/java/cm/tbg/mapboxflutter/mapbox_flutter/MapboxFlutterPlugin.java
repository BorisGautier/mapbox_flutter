...
/**
 * Plugin for controlling a set of MapboxFlutterMap views to be shown as overlays on top of the Flutter
 * view. The overlay should be hidden during transformations or while Flutter is rendering on top of
 * the map. A Texture drawn using MapboxFlutterMap bitmap snapshots can then be shown instead of the
 * overlay.
 */
public class MapboxFlutterPlugin implements FlutterPlugin, ActivityAware {
  private static final String VIEW_TYPE = "plugins.flutter.io/mapbox_flutter";

  static FlutterAssets flutterAssets;
  private Lifecycle lifecycle;

  public MapboxFlutterPlugin() {
    // no-op
  }
  ...
  public void onAttachedToEngine(@NonNull FlutterPluginBinding binding) {
    ...
    MethodChannel methodChannel = new MethodChannel(binding.getBinaryMessenger(), "plugins.flutter.io/mapbox_flutter");
    methodChannel.setMethodCallHandler(new GlobalMethodHandler(binding));

    binding
            .getPlatformViewRegistry()
            .registerViewFactory(
                    "plugins.flutter.io/mapbox_flutter", new MapboxFlutterMapFactory(binding.getBinaryMessenger(), () -> lifecycle));
  }
  ...
  public static void registerWith(PluginRegistry.Registrar registrar) {
    ...
    if (activity instanceof LifecycleOwner) {
      ...
      registrar
              ...
              .registerViewFactory(
                      VIEW_TYPE,
                      new MapboxFlutterMapFactory(registrar.messenger(), new ProxyLifecycleProvider(activity)));
    }

    MethodChannel methodChannel = new MethodChannel(
            registrar.messenger(),
            "plugins.flutter.io/mapbox_flutter"
    );
